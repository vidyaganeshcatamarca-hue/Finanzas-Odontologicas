...
export function formatPercentSmart(value) {
  if (value === null || value === undefined || isNaN(value)) return "—";
  const num = Number(value);
  // Un porcentaje como ratio suele estar entre -10 y 10 (ej: 1.24 = 124%).
  // Si el valor absoluto es >= 10, asumimos que ya viene como porcentaje (ej: 85 -> 85%).
  // Si es < 10, lo tratamos como ratio y multiplicamos x 100.
...
export function normalizePercent(value) {
  ...
  // Si es > 10, probablemente sea un valor de porcentaje (ej: 85), dividir por 100.
  // Si es < 10, probablemente sea un ratio (ej: 0.85 o 1.24), dejarlo como está.

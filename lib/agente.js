...
function buildMatrizBCG(tratamientos = []) {
  ...
  // Agrupar en cuadrantes — cada item: { nombre, tamano_burbuja }
  // tamano_burbuja = margen_total -> indica magnitud relativa
  const cuadrantes = { estrellas: [], vacas: [], dilemas: [], perros: [] };

  for (const t of activos) {
    const q = getQuadrant(t.cantidad, t.margenUnitario, medVol, medMrg);
    const item = {
      nombre:        t.nombre,
      tamano_burbuja: roundInt(t.margenTotal),
    };
    if (q === "estrella") cuadrantes.estrellas.push(item);
    else if (q === "vaca")  cuadrantes.vacas.push(item);
    else if (q === "dilema") cuadrantes.dilemas.push(item);
    else cuadrantes.perros.push(item);
  }

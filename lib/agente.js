...
// ── Builder: Matriz BCG agrupada por cuadrante ──────────────────────────────
// IMPORTANTE: la lógica de clasificación replica EXACTAMENTE la de Produccion.jsx
// El punto de corte es el PUNTO MEDIO DEL RANGO: (max + min) / 2
// NO es la mediana estadística (percentil 50)
function buildMatrizBCG(tratamientos = []) {
  const activos = tratamientos.filter((t) => t.cantidad > 0);
  if (!activos.length) return null;

  // Tomar solo Top 20 por margenTotal (mismo scope que el gráfico)
  ...
  // Punto de corte = midpoint del RANGO (igual que Produccion.jsx líneas 94-95)
  const midVolumen = volumes.length  > 0 ? (Math.max(...volumes)  + Math.min(...volumes))  / 2 : 0;
  const midMargen  = marginsU.length > 0 ? (Math.max(...marginsU) + Math.min(...marginsU)) / 2 : 0;
  ...
  // Clasificar y agrupar — igual que Produccion.jsx getQuadrant (líneas 33-38)
  // volumen > mid → alto; margenUnitario > mid → alto
  const cuadrantes = { estrellas: [], vacas: [], dilemas: [], perros: [] };

  for (const t of top20) {
    const altaVol  = t.cantidad       > midVolumen;
    const altoMrg  = t.margenUnitario > midMargen;

    let key;
    if (altaVol  && altoMrg)  key = "estrellas";
    else if (!altaVol && altoMrg)  key = "dilemas";
    else if (altaVol  && !altoMrg) key = "vacas";
    else key = "perros";

    cuadrantes[key].push({
      nombre:         t.nombre,
      tamano_burbuja: roundInt(t.margenTotal),
    });
  }

  // Ordenar cada cuadrante por tamano_burbuja desc
  for (const k of Object.keys(cuadrantes)) {
    cuadrantes[k].sort((a, b) => b.tamano_burbuja - a.tamano_burbuja);
  }

  return {
    total_tratamientos_activos: activos.length,
    punto_corte_volumen:        roundInt(midVolumen),
    punto_corte_margen_unitario: roundInt(midMargen),
...
export function buildAgentePayload(ctx) {
  ...
  return {
    ...
    metricas_actuales: {
      ...
      margen_rentabilidad:               round4(margenRentabilidad * 100),
      margen_seguridad:                  round4((msCalculado ?? 0) * 100),
      indice_cobrabilidad:               round4((kpis?.indiceCobrabilidad ?? 0) * 100),
    ...
    historico_referencia: {
      mes_anterior_cerrado: prev1 ? {
        ...
        margen_seguridad_final:    round4((prev1.margenSeguridad ?? 0) * 100),
        indice_cobrabilidad_final: round4((prev1.indiceCobrabilidad ?? 0) * 100),
        margen_rentabilidad_final: round4((prev1.margenRentabilidad ?? 0) * 100),
      } : null,
      ...
      trimestre_previo: trimMeses.length > 0 ? {
        ...
        tendencia_rentabilidad: {
          rentabilidad_promedio: round4(rentabilidadProm * 100),
          tendencia:             round4(tendencia * 100),
        },
        ms_promedio: round4(msPromedio * 100),
      } : null,
    },
